  ...
  if new.invite_token is null or new.invite_token = '' then
    loop
      ...
      exit when not exists (
        select 1
        from public.trips
        where invite_token = new.invite_token
      );
    end loop;
  end if;
...
declare
  ...
  member_trip_id uuid;
begin
  select trip_id into question_trip_id
...
create table if not exists public.expenses (
  ...
  foreign key (paid_by, trip_id) references public.trip_members(id, trip_id) on delete restrict
);
...
for each row
execute function public.set_trip_invite_token();
...
for each row
execute function public.validate_survey_response_trip_member();
...
for each row
execute function public.validate_expense_split_trip_member();
...
create policy "Users can create themselves"
on public.users
for insert
...
create policy "Creators can create trips"
on public.trips
for insert
...
create policy "Trip creators can update trip members"
on public.trip_members
for update
...
create policy "Trip members can read survey responses"
...
using (
  exists (
    ...
    where sq.id = survey_responses.question_id
      and public.is_trip_member(sq.trip_id)
  )
);

drop policy if exists "Trip members can submit survey responses" on public.survey_responses;
create policy "Trip members can submit survey responses"
on public.survey_responses
for insert
with check (
  exists (
...
create policy "Trip members can update own survey responses"
on public.survey_responses
for update
using (
  exists (
    select 1
    from public.trip_members tm
    where tm.id = survey_responses.member_id
      and tm.user_id = auth.uid()
  )
)
with check (
...
create policy "Trip members can read availability"
on public.availability
for select
using (public.is_trip_member(trip_id));

drop policy if exists "Trip members can manage own availability" on public.availability;
create policy "Trip members can manage own availability"
on public.availability
for all
using (
  exists (
    select 1
    from public.trip_members tm
    where tm.id = availability.member_id
      and tm.trip_id = availability.trip_id
      and tm.user_id = auth.uid()
  )
)
with check (
...
with check (
  exists (
    select 1
    from public.trip_members tm
    where tm.id = expenses.paid_by
      and tm.trip_id = expenses.trip_id
      and tm.user_id = auth.uid()
  )
);

drop policy if exists "Trip members can update own expenses" on public.expenses;
create policy "Trip members can update own expenses"
on public.expenses
for update
using (
  exists (
...
create policy "Expense payers can manage expense splits"
on public.expense_splits
for all
using (
  exists (
    ...
    join public.trip_members split_member on split_member.id = expense_splits.member_id
    where e.id = expense_splits.expense_id
      and split_member.trip_id = e.trip_id
